...
parser.add_argument(
    ...
    type=str,
    default="./output",
    help="The normalized xy-coordinates of the start and end locations.",
)
...
def bytes_to_pil(bytes_str):
    image = Image.open(io.BytesIO(bytes_str)).convert("RGB")
    return image
...
def analyze_dtw(sequence1, sequence2):
    ...
    normalized_distance = distance / path_length if path_length > 0 else 0
    quartile_index = path_length // 4
    start_path_segment = path[: quartile_index + 1]
    end_path_segment = path[-quartile_index - 1 :]

    metrics = {
        "dtw_distance": distance,
        "path_length": path_length,
        "path_start_indices": path[0],
        "path_end_indices": path[-1],
    }
    return metrics
...
def generate_response(
    prompts: list[str],
    image_data: Union[list[bytes], None] = None,
    processor: AutoProcessor = processor,  # No longer used for generation, but kept for compatibility
    model_name: str = None,
    max_new_tokens: int = 1024,
) -> list[str]:
    """
    Generates a batch of responses using the vLLM engine with the correct
    multi-modal API (placeholders and the `multi_modal_data` argument).
    """
    ...
    sampling_params = SamplingParams(
        temperature=0.0 if args.lora_path else 0.0,
    ...
    if image_data:
        mm_data_list = [{"image": bytes_to_pil(img_bytes)} for img_bytes in image_data]

        if "gemma" in model_name.lower():
            # Gemma uses <img> placeholder before the text
            messages = [
                [
                    {
                        "role": "user",
            ...
            text_prompts = [processor.apply_chat_template(m) for m in messages]
            ...
            raise ValueError(
                f"Model name {model_name} not supported for multi-modal generation."
            )
        inputs = [
            {"prompt": p, "multi_modal_data": m}
            for p, m in zip(text_prompts, mm_data_list)
        ]
    ...
    # Handle the text-only case
        ...
        if "gemma" in model_name.lower():
            ...
            # A generic fallback for other text-only models
    ...
    cleaned_responses = [output.outputs[0].text.strip() for output in outputs]
...
def save_print_results(all_results, total_metrics, count, output_filename):
    ...
    print(f"Average DTW distance: {total_metrics['dtw_distance']/count:.2f}")
...
def process_batch(batch_tasks):
    ...
    for item in batch_tasks:
        ...
        try:
            graph = pickle.load(
                open(os.path.join(annotations_path, f"{map_class}/{image_id}.pkl"), "rb")
            )
            label2coords = {
                node[1]["label"]: node[0]
            ...
            start_loc_normalized = (start_loc[0] / width, start_loc[1] / height)
            end_loc = label2coords[query["destination"]]
            end_loc_normalized = (end_loc[0] / width, end_loc[1] / height)
            ground_truth_path = nx.shortest_path(
                graph,
                label2coords[query["start"]],
                label2coords[query["destination"]],
            )
        ...
        curr_img = image if args.no_marks else draw_start_end_points(image, start_loc, end_loc)
        ...
        if args.precision > 0:
            start_loc = (
                round(start_loc[0], args.precision),
                round(start_loc[1], args.precision),
            )
            end_loc = (
                round(end_loc[0], args.precision),
        ...
        if args.absolute:
            ...
            curr_prompt = "".join(PROMPT).format(
                start_loc_normalized, end_loc_normalized
            )
    ...
    try:
        responses = generate_response(
            prompts=prompts, image_data=images, model_name=args.model_id
        )
    except Exception as e:
    ...
    for i, response in enumerate(responses):
        ...
        try:
            ...
            if args.absolute:
                ...
                parsed_response = [
                    (p[0] / img_width, p[1] / img_height) for p in parsed_response
                ]
            ...
            result = {
                ...
                "map_class": item_data["map_class"],
                "query": item_data["query"],
                "query_idx": item_data["query_idx"],
                "response": parsed_response,
                "ground_truth_path": item_data["normalized_ground_truth_path"],
...
if __name__ == "__main__":
    ...
    valid_results = []
    for res in results:
        if res and res.get("dtw_metrics", {}).get("dtw_distance", float("inf")) <= 10000:
    ...
    for i, row in dataset.iterrows():
        for idx, query in enumerate(row["queries"]):
            if (
                tuple((row["image_id"], query["start"], query["destination"]))
                in exists
            ):
                continue
                ...
                if count > 0:
                    ...
                    for res in current_results:
                        for key in temp_total_metrics:
                            if (
                                key in res["dtw_metrics"]
                                and res["dtw_metrics"][key] < float("inf")
                            ):
                                temp_total_metrics[key] += res["dtw_metrics"][key]
                    save_print_results(
                        current_results, temp_total_metrics, count, output_path
                    )
    ...
    if count > 0:
        ...
        save_print_results(all_results, final_total_metrics, count, output_path)
    else:
